...
type Config struct {
	...
	SignerName string
	// Duration is the requested lifetime length for certificates generated by the manager.
...
func (m *manager) rotateCerts() (bool, error) {
	...
	// new private key.
	reqName, reqUID, err := csr.RequestCertificate(clientSet, csrPEM, "", m.signerName, m.duration, m.usages, privateKey)
	if err != nil {
