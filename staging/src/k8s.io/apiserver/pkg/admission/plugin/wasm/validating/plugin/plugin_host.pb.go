...
type ValidationPluginOption struct {
	Stdout io.Writer
	Stderr io.Writer
	FS     fs.FS
}
...
func NewValidationPlugin(ctx context.Context, opt ValidationPluginOption) (*ValidationPlugin, error) {
...
func (p *ValidationPlugin) Load(ctx context.Context, pluginPath string) (Validation, error) {
	...
	apiVersion := module.ExportedFunction("validation_api_version")
	if apiVersion == nil {
		return nil, errors.New("validation_api_version is not exported")
	}
	results, err := apiVersion.Call(ctx)
	if err != nil {
		return nil, err
	} else if len(results) != 1 {
		return nil, errors.New("invalid validation_api_version signature")
	}
	if results[0] != ValidationPluginAPIVersion {
		return nil, fmt.Errorf("API version mismatch, host: %d, plugin: %d", ValidationPluginAPIVersion, results[0])
	}

	validate := module.ExportedFunction("validation_validate")
	if validate == nil {
		return nil, errors.New("validation_validate is not exported")
	}
	...
	return &validationPlugin{module: module,
		malloc:   malloc,
		free:     free,
		validate: validate,
	}, nil
...
func (p *validationPlugin) Validate(ctx context.Context, request ValidateRequest) (response ValidateResponse, err error) {
