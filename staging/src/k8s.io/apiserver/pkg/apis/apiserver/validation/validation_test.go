...
func TestValidateAuthenticationConfiguration(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "valid authentication configuration with default disallowed issuer",
			in: &api.AuthenticationConfiguration{
				JWT: []api.JWTAuthenticator{
					{
						Issuer: api.Issuer{
			...
			want: `jwt[0].issuer.url: Invalid value: "https://kubernetes.default.svc.cluster.local": URL must not overlap with disallowed issuers: [https://kubernetes.default.svc https://kubernetes.default.svc.cluster.local]`,
		},
		{
			...
			want:              `jwt[0].issuer.url: Invalid value: "https://issuer-url": URL must not overlap with disallowed issuers: [a b c https://issuer-url https://kubernetes.default.svc https://kubernetes.default.svc.cluster.local]`,
		},
		{
...
func TestValidateURL(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			got := validateURL(tt.in, tt.disallowedIssuers, fldPath).ToAggregate()
			if d := cmp.Diff(tt.want, errString(got)); d != "" {
