...
func NewGenericWebhook(scheme *runtime.Scheme, codecFactory serializer.CodecFactory, config *rest.Config, groupVersions []schema.GroupVersion, retryBackoff wait.Backoff) (*GenericWebhook, error) {
	return newGenericWebhook(scheme, codecFactory, config, groupVersions, retryBackoff, defaultRequestTimeout)
}

func newGenericWebhook(scheme *runtime.Scheme, codecFactory serializer.CodecFactory, config *rest.Config, groupVersions []schema.GroupVersion, retryBackoff wait.Backoff, requestTimeout time.Duration) (*GenericWebhook, error) {
	...
	// Kubeconfigs (which are commonly used to create the input rest.Config) cannot set a timeout,
	// this can only be set through a command line flag.
	//
