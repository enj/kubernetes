...
func TestToken(t *testing.T) {
	...
	tests := []claimsTest{
		...
		},
		{
			...
			wantInitErr: `issuer.url: Invalid value: "https://auth.example.com": URL must not overlap with disallowed issuers: [https://auth.example.com https://kubernetes.default.svc https://kubernetes.default.svc.cluster.local]`,
		},
		{
			name: "disallowed issuer via default value",
			options: Options{
				JWTAuthenticator: apiserver.JWTAuthenticator{
					Issuer: apiserver.Issuer{
			...
			wantInitErr: `issuer.url: Invalid value: "https://kubernetes.default.svc": URL must not overlap with disallowed issuers: [https://kubernetes.default.svc https://kubernetes.default.svc.cluster.local]`,
		},
		{
