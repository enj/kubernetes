...
var map_CertificateSigningRequest = map[string]string{
	...
	"spec":   "spec contains the certificate request, and is immutable after creation. Only the request, signerName, durationHint, and usages fields can be set on creation. Other fields are derived by Kubernetes and cannot be modified by users.",
...
var map_CertificateSigningRequestSpec = map[string]string{
	...
	"durationHint": "durationHint is a hint to the signer in regards to when the issued certificate should expire. The signer may or may not honor this field.  The well-known kubernetes signers will honor this field as long as the requested duration is not later than the maximum duration they will honor.",
