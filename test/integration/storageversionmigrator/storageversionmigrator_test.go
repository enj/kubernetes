...
func TestStorageVersionMigrationWithCRD(t *testing.T) {
	...
	forceCleanup := svmTest.createChaos(ctx, t)
	...
	forceCleanup() // stop chaos before shutting down conversion webhook which is handling watch events generated by the noise
	shutdownServer()
...
func TestStorageVersionMigrationDuringChaos(t *testing.T) {
	...
	_ = svmTest.createChaos(ctx, t)

	crd := svmTest.createCRD(t, crdName, crdGroup, nil, v1CRDVersion)

	crVersions := make(map[string]versions)

	for i := range 20 { // a more realistic number of total resources
	...
	const migrations = 4 // more than the total workers of SVM
	wg.Add(migrations)
