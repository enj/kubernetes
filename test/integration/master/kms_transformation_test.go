...
func (r envelope) plainTextPayload(secretETCDPath string) ([]byte, error) {
	...
	plainSecret, _, err := transformer.TransformFromStorage(r.cipherTextPayload(), ctx)
	if err != nil {
...
// 6. Old data stored in etcd with AES CBC transform is still readable and is migrated to GCM on write
...
`
	...
	legacyEncryptedSecretBytes, err := base64.StdEncoding.DecodeString(legacyEncryptedSecret)
	if err != nil {
		t.Fatalf("failed to base64 decode legacy secret, err: %v", err)
	}
	_, err = test.writeRawRecordToETCD(secretETCDPath, legacyEncryptedSecretBytes)
	if err != nil {
		t.Fatalf("failed to write legacy encrypted secret, err: %v", err)
	}
	...
	legacyKeyBytes, err := base64.StdEncoding.DecodeString(legacyAESCBCKey)
	if err != nil {
		t.Fatalf("failed to base64 decode legacy key, err: %v", err)
	}
	failingLegacyEnvelope := envelope{
		providerName: providerName,
		rawEnvelope:  failingRawEnvelope,
		plainTextDEK: legacyKeyBytes,
	}
	failingLegacyPlainSecret, err := failingLegacyEnvelope.plainTextPayload(secretETCDPath)
	if err == nil || err.Error() != "failed to transform from storage via AESGCM, err: cipher: message authentication failed" {
		t.Fatalf("AESGCM decryption failure not seen, err: %v, data: %s", err, string(failingLegacyPlainSecret))
	}

	// AES CBC secrets should be un-enveloped on direct reads from Kube API Server.
	legacySecret, err := secretClient.Get(testSecret, metav1.GetOptions{})
	if err != nil {
		t.Fatalf("failed to read legacy secret via Kube API, err: %v", err)
	}
	if legacySecretVal != string(legacySecret.Data[secretKey]) {
		t.Fatalf("expected %s from KubeAPI, but got %s", legacySecretVal, string(legacySecret.Data[secretKey]))
	}

	// no-op update should cause new AES GCM key to be used
	legacySecretUpdated, err := secretClient.Update(legacySecret)
	if err != nil {
		t.Fatalf("failed to update legacy secret via Kube API, err: %v", err)
	}
	if legacySecret.ResourceVersion == legacySecretUpdated.ResourceVersion {
		t.Fatalf("legacy secret not updated on no-op write: %s", legacySecret.ResourceVersion)
	}

	// confirm that direct AES GCM decryption works
	legacyRawEnvelope, err := test.getRawSecretFromETCD()
	if err != nil {
		t.Fatalf("failed to read %s from etcd: %v", secretETCDPath, err)
	}
	legacyEnvelope := envelope{
		providerName: providerName,
		rawEnvelope:  legacyRawEnvelope,
		plainTextDEK: pluginMock.LastEncryptRequest(),
	}
	if !bytes.HasPrefix(legacyRawEnvelope, []byte(wantPrefix)) {
		t.Fatalf("expected secret to be prefixed with %s, but got %s", wantPrefix, legacyRawEnvelope)
	}
	legacyPlainSecret, err := legacyEnvelope.plainTextPayload(secretETCDPath)
	if err != nil {
		t.Fatalf("failed to transform from storage via AESGCM, err: %v", err)
	}
	if !strings.Contains(string(legacyPlainSecret), legacySecretVal) {
		t.Fatalf("expected %q after decryption, but got %q", legacySecretVal, string(legacyPlainSecret))
	}
}
